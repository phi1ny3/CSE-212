...
def _insert_middle(sorted_list, first, last, bst):
    ...
    """
    # base condition in recursive function to check if list is not empty
    if not sorted_list:
        return None
    # base condition, if first become more than last, this occurs when sub-list becomes empty
    if first>last:
        return None
    ...
    # insert the middle element to the bst
    bst.insert(sorted_list[mid])
    
    # recursively add the left elements of the mid element to the bst, hence first and mid-1 is used to get the left sub-list of mid element
    # left subtree of root has all values <arr[mid]
    _insert_middle(sorted_list,first,mid-1,bst)
    
    # recursively add the right elements of the mid element to the bst, hence mid+1 and last is used to get the right sub-list of mid element
